...
output_dir = os.path.join(BASE_DIR, "results/large_scale/35sens")
os.makedirs(output_dir, exist_ok=True)

# ––––– INSERT just after you define  'config' –––––
# THG prices you want to sweep (€/t CO₂-eq)
THG_GRID = [75, 50, 25, 0]          # descending order
master_fin = []                        # will collect every run’s financials
master_in  = []                        # will collect every run’s inflows
warm_path  = os.path.join(output_dir, "warmstart.sol")
...
if __name__ == '__main__':
    for P_THG in THG_GRID:
        print(f"\n=== Solving for THG price = {P_THG} €/tCO2e ===")

        # 2.1  update price-dependent parameters
        GHG_certificate_price = P_THG              # overwrite global
        premium = {f: max(0, (alpha_GHG_comp - feed_yield[f]['GHG_intensity']))
                     * (alphaHV * 3.6) * GHG_certificate_price / 1e6
                   for f in feedstock_types}

        # 2.2  build and solve model
        m, Omega, N_CH4, x, y, m_up, UpgSel, \
        Rev_loc, Cost_loc, Capex_site, TotalRev, TotalCost, \
        FeedstockCost, GHG_rev, TotalCapex, Rev_alt_selected, \
        Cost_alt_selected, FeedstockCostPerPlant, \
        BaseFeedstockCost, LoadingCost, TransportCost, \
        DigestateCost, bonus_expr = build_model(config)

        # optional warm-start
        if P_THG != THG_GRID[0] and os.path.isfile(warm_path):
            m.update()                 # <- make names visible
            m.read(warm_path)
        ...
        if m.status != GRB.OPTIMAL:
            print("No optimal solution");  continue
        ...
        tag = f"THG{P_THG}"
        fin_path = os.path.join(output_dir, f"Output_financials_{tag}.csv")
        in_path  = os.path.join(output_dir, f"Output_in_flow_{tag}.csv")
        fin_df.to_csv(fin_path, index=False)
        in_flow_df.to_csv(in_path, index=False)

        # store in master lists for later plotting in Python
        fin_df["THG_price"] = P_THG
        in_flow_df["THG_price"] = P_THG
        master_fin.append(fin_df)
        master_in.append(in_flow_df)

        # save warm-start for next scenario
        m.write(warm_path)
        print(f"Scenario {tag} finished  –  NPV = {m.objVal:,.0f} €")

    # 2.4  concatenate & save once for all scenarios
    all_fin = pd.concat(master_fin, ignore_index=True)
    all_in  = pd.concat(master_in,  ignore_index=True)

    all_fin.to_csv(os.path.join(output_dir, "All_financials.csv"), index=False)
    all_in.to_csv(os.path.join(output_dir,  "All_inflows.csv"),   index=False)
    print("\nSweep complete – aggregated CSVs written.")
