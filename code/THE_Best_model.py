...
capacity_levels = (50_000_000, 60_000_000, 75_000_000)
FLH_max = 8000
alphaHV = 9.97
CN_min = 20.0
CN_max = 30.0
            ...
            for c in capacity_levels:
                ...
                if not alt["EEG_flag"]:
                    if alt["category"] == "CHP_nonEEG":
                        rev = N_CH4_max * (alphaHV / 1000) * (chp_elec_eff * electricity_spot_price + chp_heat_eff * heat_price) / 30
                    elif alt["category"] == "boiler":
                        rev = N_CH4_max * (alphaHV / 1000) * chp_heat_eff * heat_price / 30
                    elif alt["category"] == "Upgrading":
                        CO2_max = N_CH4_max  # crude estimate for CO2 share
                        rev = (N_CH4_max * gas_price_m3 + CO2_max * co2_price) / 40
                    else:
                        rev = 0
                else:
                    effective_EEG = alt["rev_price"]["EEG"] * avg_discount
                    if alt["category"] in ["EEG_CHP_small", "EEG_CHP_large"]:
                        rev = N_CH4_max * (alphaHV / 1000) * (chp_elec_eff * effective_EEG + chp_heat_eff * heat_price) / 30
                        ...
                        rev = (EEG_rev + spot_rev + heat_rev + bonus) /40
                    else:
                        rev = 0
                ...
                print(f"  M_rev[{j}, {a}, {c}] = {M_rev[key]*1e6:,.6f} €")
                print(f"  M_cost[{j}, {a}, {c}] = {M_cost[key]*1e6:,.6f} €")
...
if __name__ == '__main__':
    ...
    m, Omega, N_CH4, x, Y, m_up, Rev_loc, Cost_loc, Capex, TotalRev, TotalCost, FeedstockCost, GHGRevenue, TotalCapex,Z_rev, bonus_dict, Rev_alt_selected, Cost_alt_selected, FeedstockCostPerPlant,BaseFeedstockCost, LoadingCost, TransportCost, DigestateCost = build_model(config)
    m.update()
