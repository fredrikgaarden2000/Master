...
def build_model(config):
    ...
    warmstart_dir = r"C:\Clone\Master\results\large_scale_cont\10_greedy_with_alternatives\greedy_reopt"
    financials_path = os.path.join(warmstart_dir, "Financials_Reopt.csv")
    flows_path = os.path.join(warmstart_dir, "Flows_Reopt.csv")

    # Load Financials_Reopt.csv
    financials_df = pd.read_csv(financials_path)
    financials_df['PlantLocation'] = financials_df['PlantLocation'].astype(str)

    # Load Flows_Reopt.csv
    flows_df = pd.read_csv(flows_path)
    flows_df['SupplyNode'] = flows_df['SupplyNode'].astype(str)
    flows_df['PlantLocation'] = flows_df['PlantLocation'].astype(str)
    flows_df['Feedstock'] = flows_df['Feedstock'].astype(str)

    # Create mappings for warm-start
    plant_alternative_map = {}
    plant_capacity_map = {}
    for _, row in financials_df.iterrows():
        j = str(row['PlantLocation'])
        alt_name = row['Alternative']
        capacity = row['Capacity'] / 1e6  # Convert to Mm³
        plant_alternative_map[j] = next(a for a, alt in enumerate(alternative_configs) if alt['name'] == alt_name)
        plant_capacity_map[j] = capacity

    feedstock_flow_map = {}
    for _, row in flows_df.iterrows():
        i = str(row['SupplyNode'])
        f = str(row['Feedstock'])
        j = str(row['PlantLocation'])
        flow = row['FlowTons'] / 1e6  # Convert to Mm³
        feedstock_flow_map[(i, f, j)] = flow

    # Set warm-start for y[j, a]
    for j in plant_locs:
        if j in plant_alternative_map:
            a = plant_alternative_map[j]
            y[j, a].Start = 1.0
            for other_a in range(len(alternative_configs)):
                if other_a != a:
                    y[j, other_a].Start = 0.0
        else:
            for a in range(len(alternative_configs)):
                y[j, a].Start = 0.0

    # Set warm-start for Omega[j]
    for j in plant_locs:
        if j in plant_capacity_map:
            Omega[j].Start = plant_capacity_map[j]
        else:
            Omega[j].Start = 0.0

    # Set warm-start for x[i, f, j]
    for i in supply_nodes:
        for f in feedstock_types:
            for j in plant_locs:
                if (i, f, j) in feedstock_flow_map:
                    x[i, f, j].Start = feedstock_flow_map[(i, f, j)]
                else:
                    x[i, f, j].Start = 0.0

    # --- auction caps -----------------------------------------------------------
    idx_eeg_biogas     = [a for a,alt in enumerate(alternative_configs)
                        if alt["category"] == "FlexEEG_biogas"]
    idx_eeg_biomethane = [a for a,alt in enumerate(alternative_configs)
                        if alt["category"] == "FlexEEG_biomethane"]
# --- Flex-EEG biogas cap -----------------------------------------------
    if idx_eeg_biogas:                     # list is non-empty
        m.addConstr(
            gp.quicksum(Omega[j] *
                        gp.quicksum(y[j, a] for a in idx_eeg_biogas)
                        for j in plant_locs)
            <= auction_chp_limit,
            name="Auction_FlexEEG_biogas")

    # --- Flex-EEG biomethane cap -------------------------------------------
    if idx_eeg_biomethane:
        m.addConstr(
            gp.quicksum(Omega[j] *
                        gp.quicksum(y[j, a] for a in idx_eeg_biomethane)
                        for j in plant_locs)
            <= auction_bm_limit,
            name="Auction_FlexEEG_biomethane")

    # inside build_model, immediately after you create Omega:
    ...
    idx_need_upgrading = [a for a,alt in enumerate(alternative_configs)
                      if alt["category"] in ("Upgrading", "FlexEEG_biomethane")]

    for j in plant_locs:
        m.addConstr(
            UpgSel[j] == gp.quicksum( y[j,a] for a in idx_need_upgrading )
        )
    ...
    raw_upg = [ (b*1e6/FLH_max) * cu * safe_pow(b*1e6/FLH_max, eu) / 1e6
            for b in BREAKS ]
    # now add +1 to every segment
    ...
    for j in plant_locs:
        ...
        for a, alt in enumerate(alternative_configs):
            ...
            if not alt["EEG_flag"]:
                ...
                if alt["category"] == "Upgrading":
                    rev_val  = N_CH4[j] * alt["rev_price"]["gas"] \
                            + (Omega[j] - N_CH4[j]) * alt["rev_price"]["co2"]
                ...
                # 2) Non-EEG CHP
                    ...
                    # heat
                    heat_rev = N_CH4[j] * (chp_heat_eff * alphaHV / 1000.0) \
                            * alt["rev_price"]["heat"]
                    rev_val  = spot_rev + heat_rev
                    # no extra variable cost beyond your PWL opex
                    cost_val = 0
                ...
                # 4) EEG biomethane (if you also implement it)
            ...
            # link into your indicator‐vars
            m.addGenConstrIndicator(
                y[j, a], True,  Rev_alt_selected[j, a] == rev_val,  name=f"Rev_on_{j}_{a}"
            )
            m.addGenConstrIndicator(
                y[j, a], False, Rev_alt_selected[j, a] == 0,        name=f"Rev_off_{j}_{a}"
            )
            m.addGenConstrIndicator(
                y[j, a], True,  Cost_alt_selected[j, a] == cost_val, name=f"Cost_on_{j}_{a}"
            )
            m.addGenConstrIndicator(
                y[j, a], False, Cost_alt_selected[j, a] == 0,        name=f"Cost_off_{j}_{a}"
            )
...
if __name__ == '__main__':
    ...
    m, Omega, N_CH4, x, y, m_up, UpgSel,Rev_loc, Cost_loc, Capex_site, TotalRev, TotalCost, FeedstockCost, GHG_rev, TotalCapex, Rev_alt_selected, Cost_alt_selected, FeedstockCostPerPlant, BaseFeedstockCost, LoadingCost, TransportCost, DigestateCost, bonus = build_model(config)
    m.update()
