...
# Configuration for fixing alternatives and capacities
config = {
    "name": "DynamicPlants",
    "eeg_enabled": False,
    "supply_enabled": True,
    ...
    "flh_enabled": True,
    "fix_alternative": True,  # Set to True to lock alternatives from Financials_20.csv
    "fix_capacity": False     # Set to True to lock capacities from Financials_20.csv
}
...
output_dir = os.path.join(BASE_DIR, f"results/large_scale_cont/10_greedy_with_alternatives/greedy_reopt/optimal_no_EEG_dynamic_alt_{config['fix_alternative']}_cap_{config['fix_capacity']}")
...
financials_df = pd.read_csv(f"{BASE_DIR}results/large_scale_cont/10_greedy_with_alternatives/greedy/Financials_20.csv")
distance_df = pd.read_csv(f"{BASE_DIR}Distance_Matrix_20.csv")
yields_df = pd.read_csv(f"{BASE_DIR}Feedstock_yields.csv")

# Filter feedstock data
...
# Filter plant locations based on Financials_20.csv if fixing alternatives or capacities
if config['fix_alternative'] or config['fix_capacity']:
    valid_locations = set(financials_df['PlantLocation'])
    plant_df = plant_df[plant_df['Location'].isin(valid_locations)]
...
# Create dictionary for fixed configurations from Financials_20.csv
fixed_configs = {row['PlantLocation']: {
    'capacity': row['Capacity'] / 1e6,  # Convert to Mm³
    'alternative': row['Alternative']
} for _, row in financials_df.iterrows()}

# Validate alternatives if fixing
if config['fix_alternative']:
    valid_alts = {alt['name'] for alt in alternative_configs}
    for loc, cfg in fixed_configs.items():
        if cfg['alternative'] not in valid_alts:
            raise ValueError(f"Alternative {cfg['alternative']} for location {loc} not found in alternative_configs")
...
def build_model(config, fixed_configs):
    m = gp.Model("ShadowPlant_Biogas_Model_Dynamic")
    ...
    # Fix configurations based on flags
    for j in plant_locs:
        if j in fixed_configs:
            if config['fix_capacity']:
                m.addConstr(Omega[j] == fixed_configs[j]['capacity'], name=f"Fix_Capacity_{j}")
            if config['fix_alternative']:
                alt_idx = next(a for a, alt in enumerate(alternative_configs) if alt['name'] == fixed_configs[j]['alternative'])
                m.addConstr(y[j, alt_idx] == 1, name=f"Fix_Alt_{j}")
                for a in range(len(alternative_configs)):
                    if a != alt_idx:
                        m.addConstr(y[j, a] == 0, name=f"Disable_Alt_{j}_{a}")

    # Auction caps
    idx_eeg_biogas = [a for a, alt in enumerate(alternative_configs) if alt["category"] == "FlexEEG_biogas"]
    idx_eeg_biomethane = [a for a, alt in enumerate(alternative_configs) if alt["category"] == "FlexEEG_biomethane"]
    if idx_eeg_biogas:
        m.addConstr(
            gp.quicksum(Omega[j] * gp.quicksum(y[j, a] for a in idx_eeg_biogas) for j in plant_locs)
            <= auction_chp_limit,
            name="Auction_FlexEEG_biogas")
    if idx_eeg_biomethane:
        m.addConstr(
            gp.quicksum(Omega[j] * gp.quicksum(y[j, a] for a in idx_eeg_biomethane) for j in plant_locs)
            <= auction_bm_limit,
            name="Auction_FlexEEG_biomethane")

    # Bonus for EEG plants
    bonus = m.addVars(plant_locs, lb=0, name="bonus")
    BONUS_RATE = (100 * system_methane_average * chp_elec_eff * alphaHV / FLH_max)
    for j in plant_locs:
        m.addConstr(bonus[j] == BONUS_RATE * Omega[j], name=f"Bonus_def_{j}")
    ...
    # Logical constraints (only if not fixing alternatives)
    if not config['fix_alternative']:
        m.addConstrs(
    ...
    idx_need_upgrading = [a for a, alt in enumerate(alternative_configs) if alt["category"] in ("Upgrading", "FlexEEG_biomethane")]
    for j in plant_locs:
        m.addConstr(UpgSel[j] == gp.quicksum(y[j, a] for a in idx_need_upgrading))

    # Upgrading production
    for j in plant_locs:
        for f in feedstock_types:
            prod_f = gp.quicksum(x[i, f, j] * feed_yield[f]['biogas_m3_per_ton'] * feed_yield[f]['ch4_content'] for i in supply_nodes)
            m.addGenConstrIndicator(UpgSel[j], True, m_up[j, f] == prod_f, name=f"UpgProd_on_{j}_{f}")
            m.addGenConstrIndicator(UpgSel[j], False, m_up[j, f] == 0, name=f"UpgProd_off_{j}_{f}")
    ...
    upg_hat = m.addVars(plant_locs, name="upgCap_hat")
    upg_eff = m.addVars(plant_locs, lb=0, name="upgCap_eff")
    cu, eu = alt_upg["upg_cost_coeff"], alt_upg["upg_cost_exp"]
    raw_upg = [(b * 1e6 / FLH_max) * cu * safe_pow(b * 1e6 / FLH_max, eu) / 1e6 for b in BREAKS]
    upg_capex_vals = [1 + v for v in raw_upg]
    for j in plant_locs:
        m.addGenConstrPWL(Omega[j], base_hat[j], list(BREAKS), base_capex_vals, name=f"PWL_baseCap_{j}")
        m.addGenConstrPWL(Omega[j], upg_hat[j], list(BREAKS), upg_capex_vals, name=f"PWL_upgCap_{j}")
        m.addGenConstrIndicator(UpgSel[j], True, upg_eff[j] == upg_hat[j], name=f"UpgCap_on_{j}")
        m.addGenConstrIndicator(UpgSel[j], False, upg_eff[j] == 0, name=f"UpgCap_off_{j}")
        m.addConstr(Capex_site[j] == base_hat[j] + upg_eff[j], name=f"Capex_total_{j}")
    ...
    BIG_M = max(premium.values()) * Q_MAX * 0.7

    GHG_rev = {j: m.addVar(lb=0, name=f"GHGrev_{j}") for j in plant_locs}
    for j in plant_locs:
        m.addConstr(GHG_rev[j] == gp.quicksum(premium[f] * m_up[j, f] for f in feedstock_types), name=f"DefGHGrev_{j}")
        m.addConstr(GHG_rev[j] <= BIG_M * UpgSel[j], name=f"GHGrev_logic_{j}")
        ...
        for a, alt in enumerate(alternative_configs):
            ...
            if not alt["EEG_flag"]:
                if alt["category"] == "Upgrading":
                    ...
                    heat_rev = N_CH4[j] * (chp_heat_eff * alphaHV / 1000.0) * alt["rev_price"]["heat"]
                    rev_val = spot_rev + heat_rev
                    cost_val = 0
            else:
                ...
                if alt["category"] == "FlexEEG_biogas":
                    ...
                    U_elec = N_CH4[j] * chp_elec_eff * alphaHV / 1000.0
                    EEG_rev = cap_fraction * U_elec * effective_EEG
                    spot_rev = (U_elec - cap_fraction * U_elec) * electricity_spot_price
                    heat_rev = N_CH4[j] * chp_heat_eff * alphaHV / 1000.0 * heat_price
                    rev_val = EEG_rev + spot_rev + heat_rev + bonus[j]
    ...
    return m, Omega, N_CH4, x, y, m_up, UpgSel, Rev_loc, Cost_loc, Capex_site, TotalRev, TotalCost, FeedstockCost, GHG_rev, TotalCapex, Rev_alt_selected, Cost_alt_selected, FeedstockCostPerPlant, BaseFeedstockCost, LoadingCost, TransportCost, DigestateCost, bonus

if __name__ == '__main__':
    print(f"Running dynamic model with fix_alternative={config['fix_alternative']}, fix_capacity={config['fix_capacity']}...")
    m, Omega, N_CH4, x, y, m_up, UpgSel, Rev_loc, Cost_loc, Capex_site, TotalRev, TotalCost, FeedstockCost, GHG_rev, TotalCapex, Rev_alt_selected, Cost_alt_selected, FeedstockCostPerPlant, BaseFeedstockCost, LoadingCost, TransportCost, DigestateCost, bonus = build_model(config, fixed_configs)
    m.update()
    ...
    if m.status == GRB.OPTIMAL:
        print(f"\nDynamic model: NPV = {m.objVal:,.2f} €, Solve time = {m.Runtime:.2f} s, MIP Gap = {m.MIPGap:.4f}")
        for j in plant_locs:
            if Omega[j].X > 1e-6:
    ...
    in_flow_df.to_csv(os.path.join(output_dir, f"Output_in_flow_dynamic_alt_{config['fix_alternative']}_cap_{config['fix_capacity']}.csv"), index=False)
    ...
    fin_df.to_csv(os.path.join(output_dir, f"Output_financials_dynamic_alt_{config['fix_alternative']}_cap_{config['fix_capacity']}.csv"), index=False)

    # Warm-start solution
    warmstart_path = os.path.join(output_dir, f"warmstart_dynamic_alt_{config['fix_alternative']}_cap_{config['fix_capacity']}.sol")
    ...
    with open(f'{BASE_DIR}/Solutions/aggregated/execution_times_dynamic_alt_{config["fix_alternative"]}_cap_{config["fix_capacity"]}.txt', 'a') as f:
